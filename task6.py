...
class Home:
    ...
    def __init__(self, household_type, total_area, furniture):
        ...
        self.reamaining_area_reading()                 # 1) Автозапуск  2) Атрибут класса Home

    """ Площадь не занимаемая мебелью """
    def reamaining_area_reading(self):
        furniture_area = 0
        for value in self.furniture.values():
            furniture_area += value
        self.reamaining_area = self.total_area - furniture_area

    """ Вывод всей информации о доме """
    def print_home(self):
        print(f"Household type: \n\t\t\t\t{self.household_type.upper()}"
              f"\nTotal area: \n\t\t\t{self.total_area} кв.м"
              f"\nRemaining area: \n\t\t\t\t{self.reamaining_area} кв.м"
              f"\nFurniture name list: "
              )
        for key, values in self.furniture.items():
            print(f"\t\t\t\t\t {key.title()} - {values} кв.м")


dom = Home("elite", 200, {"bed": 4, "wardrobe": 2, "table": 1.5})
dom.print_home()
